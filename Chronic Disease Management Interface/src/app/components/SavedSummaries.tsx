...
export function SavedSummaries({ onBack, onDocumentClick, documents }: SavedSummariesProps) {
  ...
  msOverflowStyle: 'none',
        ...
        <div className="px-5 pt-3 pb-4 flex items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <h1 className="text-[34px] font-bold tracking-[0.37px] leading-tight">Saved Summaries</h1>
          </div>
        </div>
        ...
        <div className="px-5 pb-4">
          <p className="text-[15px] text-[#848484] tracking-[-0.24px] leading-[1.4]">
            {documents.length} document {documents.length === 1 ? 'summary' : 'summaries'} saved
          </p>
        </div>
          ...
          {documents.map((doc) => (
            ...
            >
              <div className="flex items-start gap-3 text-left">
                <div className="w-10 h-10 rounded-full bg-[rgba(255,149,0,0.09)] flex items-center justify-center flex-shrink-0 mt-0.5">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-[17px] font-semibold tracking-[-0.43px] text-black">
                    {doc.title}
                  </h3>
                  <p className="text-[13px] text-[#848484] tracking-[-0.08px] mt-0.5">
                    {doc.type} · {doc.date}
                  </p>
                  <p className="text-[15px] text-[#848484] tracking-[-0.24px] leading-[1.4] mt-2 line-clamp-2">
                    {doc.takeaway}
                  </p>
                </div>
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="opacity-30 flex-shrink-0 mt-1">
