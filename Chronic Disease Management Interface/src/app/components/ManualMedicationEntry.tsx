...
import { hypertensionMedications } from '../data/hypertensionMedications';

interface ManualMedicationEntryProps {
  onBack: () => void;
  onNext: (medication: { name: string; dosage: string; instructions: string }) => void;
}

export function ManualMedicationEntry({ onBack, onNext }: ManualMedicationEntryProps) {
  ...
  const suggestions = medicationName.trim().length > 0
    ? hypertensionMedications.filter(med =>
  ...
  msOverflowStyle: 'none',
        ...
        <div className="px-5 pt-3 pb-4 flex items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <h1 className="text-[34px] font-bold tracking-[0.37px] leading-tight">Enter Medication Details</h1>
          </div>
        </div>
          ...
          <div className="relative">
            <div className="bg-white rounded-[26px] p-4 shadow-sm">
              ...
              <input
                ...
                onFocus={() => medicationName.trim().length > 0 && setShowSuggestions(true)}
                placeholder="e.g., Lisinopril"
                className="w-full text-[17px] font-semibold tracking-[-0.43px] text-black bg-transparent border-none outline-none placeholder:text-[#c7c7cc]"
              />
            </div>

            {/* Autocomplete Suggestions */}
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-[20px] shadow-lg overflow-hidden z-10">
                {suggestions.map((suggestion, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSelectSuggestion(suggestion)}
                    className="w-full px-4 py-3 text-left text-[15px] tracking-[-0.24px] text-black active:bg-[#f2f2f7] transition-colors border-b border-[#e6e6e6] last:border-b-0"
                  >
                    {suggestion}
                  </button>
                ))}
      ...
      <div className="pb-8 px-4 flex-shrink-0">
        <button
          onClick={() => onNext({ name: medicationName, dosage, instructions })}
          disabled={!isValid}
        ...
        >
          <span className="text-white text-[17px] font-semibold tracking-[-0.43px]">Continue</span>
        </button>
      </div>
    </div>
  );
