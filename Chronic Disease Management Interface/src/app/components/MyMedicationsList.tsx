...
interface MyMedicationsListProps {
  onBack: () => void;
  onMedicationClick: () => void;
  medications: Medication[];
}

export function MyMedicationsList({ onBack, onMedicationClick, medications }: MyMedicationsListProps) {
  const getFrequencyText = (freq: 'morning' | 'evening' | 'both') => {
    if (freq === 'morning') return 'Once daily, 8:00 AM';
    if (freq === 'evening') return 'Once daily, 8:00 PM';
    return 'Twice daily, 8:00 AM & 8:00 PM';
  };
  ...
  msOverflowStyle: 'none',
        ...
        <div className="px-5 pt-3 pb-4 flex items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <h1 className="text-[34px] font-bold tracking-[0.37px] leading-tight">My Medications</h1>
          </div>
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[13px] text-[#848484] tracking-[-0.08px] mt-0.5">{med.dosage}</div>
                  <div className="text-[13px] text-[#848484] tracking-[-0.08px] mt-0.5">{getFrequencyText(med.frequency)}</div>
                </div>
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="opacity-30 flex-shrink-0 mt-0.5">
